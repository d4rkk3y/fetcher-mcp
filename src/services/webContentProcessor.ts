...
export class WebContentProcessor {
    ...
    try {
      ...
      try {
        ...
        if (gotoError.message.includes("Timeout") || gotoError.message.includes("timeout")) {
          logger.warn(
            `${this.logPrefix} Navigation timeout: ${gotoError.message}. Attempting to retrieve content anyway...`,
          );
          ...
          try {
            ...
            if (html && html.trim().length > 0) {
              logger.info(
                `${this.logPrefix} Successfully retrieved content despite timeout, length: ${html.length}`,
              );
            ...
            logger.error(
              `${this.logPrefix} Failed to retrieve content after timeout: ${retrieveError.message}`,
            );
          }
        }
  ...
  private async ensurePageStability(page: any): Promise<void> {
    try {
      ...
      logger.warn(
        `${this.logPrefix} Error ensuring page stability: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
    ...
    while (attempt < retries) {
      try {
        ...
        if (errorMessage.includes("Execution context was destroyed") && attempt < retries) {
          logger.warn(
            `${this.logPrefix} Context destroyed, waiting for navigation to complete (attempt ${attempt}/${retries})...`,
          );
